...
<% if (strpos($action, 'add') === false): %>
        <li><?= $this->Form->postLink(
                ...
                ['confirm' => __('Are you sure you want to delete # {0}?', $<%= $singularVar %>-><%= $primaryKey[0] %>),'class'=>'btn btn-sm btn-primary']
            )
        ?></li>
<% endif; %>
        <li><?= $this->Html->link(__('List <%= $pluralHumanName %>'), ['action' => 'index'], ['class'=>'btn btn-sm btn-primary']) ?></li>
<%
        $done = [];
